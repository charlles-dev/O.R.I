...
const FRAGMENT_SHADER = `
  ...
  vec3 getBackground(vec3 rd) {
    ...
    nebula += vec3(0.89, 0.13, 0.52) * pow(max(0.0, sin(rd.x * 2.0 + uTime * 0.1)), 3.0) * 0.25;
    nebula += vec3(0.17, 0.22, 0.57) * pow(max(0.0, sin(rd.y * 2.5 + uTime * 0.05)), 3.0) * 0.25;
    nebula += vec3(0.39, 0.96, 0.96) * pow(max(0.0, cos(rd.z * 1.8 + uTime * 0.08)), 3.0) * 0.1;
  ...
  void main() {
    ...
    if(t < MAX_DIST) {
      ...
      color += edge * vec3(0.39, 0.96, 0.96) * 0.7;

      float sss = pow(max(dot(-normal, lightDir), 0.0), 2.0);
      color += sss * vec3(0.89, 0.13, 0.52) * 0.5;
    } else {
...
export default function PrismCanvas() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      gl.clearColor(0.04, 0.024, 0.05, 1.0);
